...
INTERFACE CHAR
  MODULE PROCEDURE s_to_c, &     ! string to character
                   ...
                   sarray_to_c   ! array of strings to fixed length character array
ENDINTERFACE 
...
PUBLIC :: VARYING_STRING,VAR_STR,CHAR,LEN,GET,PUT,PUT_LINE,INSERT,REPLACE,   &
          ! Removed split from list of publically accessible entities 
          !               --- Aidan, Wed Oct  1 15:23:19 EST 2003 
          ! SPLIT, &
          REMOVE,REPEAT,EXTRACT,INDEX,SCAN,VERIFY,LLT,LLE,LGE,LGT,     &
...
!----- Conversion Procedures ------------------------------------------------! 
 ...
 function sarray_to_c(string) ! generic CHAR

   ! returns the characters of string as an automatically sized character array

   implicit none

   type(varying_string), intent(in), dimension(:)              :: string 
   character(len=maxval(len(string))), dimension(size(string)) :: sarray_to_c 

   integer :: i
   do i=1,size(string)
      sarray_to_c(i) = char(string(i))
   end do

end function sarray_to_c
