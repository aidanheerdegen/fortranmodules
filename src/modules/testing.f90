module zmatrix_class
  ...
  character(len=*), parameter :: version = "$Id: zmatrix_class.f90,v 1.11 2005/02/11 05:10:54 aidan Exp $" 
  ...
  type zmatrix_object
     private
     type(zmatrix_entry), dimension(:), pointer :: zmatrices ! => null()
...
contains
  ...
  subroutine delete_zmatrix(zmatrix)
    ...
    deallocate(zmatrix%zmatrices,stat=error)
    ! print *,'here',error
    if (error /=0) nullify(zmatrix%zmatrices)
...
1   return
  ...
  function zmatrix_from_template_mol2(template, mol2, substructure) result(zmatrix)
    ...
    if (atom_num(mol2,subst_id) /= num(template)) then
       write(stderr,"('ZMATRIX_CLASS :: Number of atoms in zmatrix template (',I0,') does not match number those in mol2 (',I0,')')") num(template),atom_num(mol2,subst_id)
    end if
  ...
  function zmatrix_from_template(template, coordinates) result(zmatrix)
    ...
    if (size(coordinates,2) /= num(template)) then
       write(stderr,"('ZMATRIX_CLASS :: Number of atoms in zmatrix template (',I0,') does not match number those in coordinate array (',I0,')')") num(template),size(coordinates,2)
       ! stop
    end if
  ...
  function makeint(cartesian, connectivity, order, labels)
    ...
    ! Local variables
    integer :: i, error, num_atoms, base_atom, second_atom, third_atom, fourth_atom, dummy, newbase
       ...
       if (i >= 2) then
          ...
          if (i >= 3) then
             ...
             do
                ...
                if (abs(abs(bond_angle*radian) - 180.) > 1.) exit

                print '(A,I0,A,F6.1)','Atom ',third_atom,' not suitable, angle = ',bond_angle*radian
             ...
             if (i >= 4) then
                ...
                nullify(skip_list)
                dummy = push(skip_list, (/base_atom, third_atom/) )
                ...
                do
                   ...
                   fourth_atom = adjacent (newbase, skip_list, makeint, connectivity)
                   ...
                   if (fourth_atom .eq. 0) then 
                      ...
                      newbase = third_atom
                      ! nullify(skip_list)
                      dummy = push(skip_list, second_atom)

                      cycle

                   end if

                   ! Calculate the bond angle between the base atom and 
                   ! the third and fourth atoms
                   bond_angle = angle(cartesian(:,base_atom),  &
                        cartesian(:,second_atom),  &
                        cartesian(:,fourth_atom))

                   ! Accept this dihedral angle if the bond angle is not 
                   ! close to 180 or 0 deg
                   if ( (abs(bond_angle*radian) > 1.) .and. (abs(abs(bond_angle*radian) - 180.) > 1.) ) exit

                   ! print '(A,I0,A,F6.1)','Atom ',fourth_atom,' not suitable, angle = ',bond_angle*radian

                   ! Push this atom on to the skip list and get another contender
                   dummy = push(skip_list, fourth_atom)
                   ...
                   ! Accept this bond angle if it is not close to 0 deg
                   ! if (abs(bond_angle*radian) > 5.) exit

                   ! Push this atom on to the skip list and get another contender
                   ! dummy = push(skip_list, fourth_atom)
                   
                makeint%zmatrices(base_atom)%dihedral_angle = bond_angle
...
1   format ("zmatrix_class :: Connectivity error -- atom id: ",I4,"   atom number: ",I4)
  ...
  pure integer function get_num_entries (zmatrix)
    ...
    get_num_entries = 0
    if (associated(zmatrix%zmatrices)) get_num_entries = size(zmatrix%zmatrices)
